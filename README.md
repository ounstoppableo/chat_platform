...
今天完成了基本的消息接收和发送，明天应该考虑如何进行数据存储
...
- 我觉得对于一个实时聊天系统，需要http请求与socket搭配使用，一个虽然也行，但是会造成代码的臃肿以及不可维护，我初步设想关于数据库查的部分交给http请求，而socket主要负责数据库修改的部分（也就是实时更新的部分），这样才能保证socket的高效性，不然socket的请求内还要进行数据库查询，那速度简直不要太慢，不过也只是刚刚开始，后续可能有新的看法
- 因为聊天系统是有订阅发布的特性的，就是一个地方改变会牵动其他地方改变，举个例子：用户进行登录/下线、发送消息，是不是都会走socket，那么我们就可以在socket集中进行状态管理，所以可以发现其实这个项目中进行状态管理基本上就只有一个集中的store,然后这个store派发各种reducer，socket在根据接收的消息进行动作派发。其实我想说的就是，到这里其实已经确定了我们项目的一个架构模式，也就是其他很多地方依赖一个集中的地方而实现状态修改到最后显示到页面上，所以在技术选型上我们也有一定的方向了。
